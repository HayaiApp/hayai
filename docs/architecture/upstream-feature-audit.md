...
| Text downloads/offline reading | J2K `DownloadQueue`, `DownloadJob`, status flow, notification, details actions, and Downloads sheet with a Hayai storage delegate for novel documents | local fixture verified on emulator for document save, Wi-Fi-only pause reason, Downloads-sheet visibility, APK reinstall and process restart recovery, and active-state preservation; remote and bulk flows pending |
...
| Translation, definition, search, and dictionary handoff | Bounded provider clients and cache; shared native/Web selection actions with retained Web anchors; Google Translate process-text/send routing; forced resizable Custom Tabs with browser login state; and a secure J2K bottom-sheet WebView fallback | port implemented; focused routing tests pending final pass and on-device floating-action verification pending |
...
| Quotes | Typed `hayai_quotes`, visible semantic Quote action in native/Web selection, retained Web anchors, duplicate-safe create, edit, attributed copy, reorder, browse, and confirmed delete UI | port implemented; emulator selection lifecycle pending |
