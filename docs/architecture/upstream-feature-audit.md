...
| Dedicated reader, central routing, and J2K history | Hayai-owned J2K-style XML shell, native `TextView` and isolated `WebView` renderers, typed actions, bounded continuous append/prepend with stable chapter blocks and visible-chapter progress/history, incognito-aware J2K state, and `ReaderLauncher` | implemented; native local-chapter launch and process restart verified; WebView, prepend, and incognito emulator flows pending |
...
| Typography, themes, spacing, navigation, presets, snippets, search/replace | Five-tab live reader sheet, Tsundoku preference keys and exact tap-zone tables, SAF-imported validated fonts shared by native/Web renderers, progress modes, ordered bottom/status registries, searchable full settings, ordered snippets with append semantics, and testable ordered replacements | implemented; all five reader tabs verified on emulator; imported-font flow pending |
