...
| Text downloads/offline reading | J2K `DownloadQueue`, `DownloadJob`, status flow, notification, details and reader actions, and Downloads sheet with a Hayai storage delegate for novel documents; explicit auto-start resumes stopped non-empty queues; a cancellable per-source pacer delays only queued metered novel saves with global and stable-source overrides | queue/offline flow previously verified on emulator; pacing policy and cancellation behavior have focused tests, while paced multi-chapter device verification remains pending |
| Typography, themes, spacing, navigation, presets, snippets, search/replace | Five Tsundoku icon tabs hosted from J2K's reader settings action; adjacent theme and text/background color controls; Tsundoku-style preset swatches plus synchronized hex/RGB picker; stable backend preference plus independent continuous/paginated layout and horizontal/vertical-rl writing direction; exact tap-zone tables; SAF-imported fonts; searchable full settings; ordered snippets and replacements | render-plan, paginated HTML, and vertical-writing CSS have focused tests; the full Android 16 settings page is crash-free for persisted integer margins; unified-reader picker, imported-font, paginated gesture, and vertical Japanese device flows remain pending |
...
| Library content selector, shortcuts, and migration compatibility | Typed content policy with an always-visible All/Manga/Novels selector in the J2K Library toolbar; shared Manga/Novel migration sections; source-unavailable legacy identity recovery; catalogue-wide novel targets; atomic progress, quote, word-count, and highlight transfer | selector persistence, stale-source classification, and migration mapping/copy/conflict behavior covered by focused tests; end-to-end library and migration emulator flows pending |
